...
面向市场与行业信息跟踪的 RSS 增量采集与 AI 情报分析系统。系统从 Google News 等 RSS 源获取候选新闻，由 CCTQ 转发的 GPT-5.4 mini 通过 OpenAI Responses API 兼容接口和内置网页搜索直接读取并保存正文，再由 DeepSeek 分阶段完成业务相关性审核与情报分析，最后按日期和关键词分类分别生成可追溯的日报。
...
- 系统设置可查看和修改企业业务边界、相关性审核与日报生成的业务提示词；固定 JSON、安全、关键词分类边界、业务分类和来源引用约束由系统追加。
