...
面向市场与行业信息跟踪的 RSS 增量采集与 AI 情报分析系统。系统从 Google News 等 RSS 源获取新闻，通过 DeepSeek 判断企业相关性、生成摘要、分类、影响与风险分析，并按日期和分类生成可追溯的情报日报。
...
- 通过 DeepSeek 结构化判断新闻与英科医疗业务的真实相关性。
- 对相关新闻生成摘要、固定分类、影响方向、风险因素和建议动作。
...
- AI 当前主要依据 RSS 标题和摘要，未抓取全文时可能因证据不足将文章判为无关。
- 当前没有历史日期范围补采和消息通知能力。
