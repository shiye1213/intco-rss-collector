...
面向市场与行业信息跟踪的 RSS 增量采集与 AI 情报分析系统。系统从 Google News 等 RSS 源获取候选新闻，由 CCTQ 转发的 GPT-5.4 mini 通过 OpenAI Responses API 兼容接口和内置网页搜索直接读取并保存正文，再由 DeepSeek 分阶段完成业务相关性审核与情报分析，最后按日期和分类生成可追溯的日报。

## 当前能力

- 管理搜索型和直连型 RSS 源。
- 管理关键词检索策略，按“主题词 × 业务信号词 − 排除词 + 回溯窗口”生成 Google News 查询表达式；运行时依据 RSS 源语言自动拆分，中文词只请求中文源，英文词只请求英文源。
...
- 按新闻发布日期与分类生成风险日报并持久化保存；关键进展、风险、机会、建议和监控项均可追溯发布方与原文。
- 保存 AI 批次日志、模型、Prompt 版本、Token 用量和原始响应，支持审计。
- 手动分析一次点击即可处理当时全部待办，并按配置的批次大小连续执行；也可配置为采集后自动分析和自动生成当天日报。
...
- 系统设置可查看和修改相关性审核与日报生成的业务提示词；固定 JSON、安全、分类和来源引用约束由系统追加。
