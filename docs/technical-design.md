...
2. 每个日报分类可同时配置核心组与拓展召回组；拓展组补充行业同义表达，并可要求标题或摘要真实命中主题词。
...
1. 用户选择北京时间日期和一个关键词分类。
2. 系统按新闻 `published_at` 筛选当日业务分析成功、且至少命中该关键词分类下一个关键词的新闻。
3. 系统组合通用日报提示词和当前关键词分类的专属提示词，再将分类名称、命中关键词、已保存的摘要、AI 业务分类、影响、风险和原文链接输入模型；其他分类的专属提示词不进入本次调用。
4. 贸易政策、关税调整和行业法规分别按政策准入、税费机制和产品合规框架分析。DeepSeek 返回管理层摘要，以及带业务分类和来源文章 ID 的关键进展、风险、机会、建议动作和观察清单。
...
8. 自动日报按活跃关键词分类逐个生成；无合格文章的分类跳过。例如“贸易政策”和“关税调整”各自形成独立日报。
...
| `keyword_categories` | 关键词分类名称、排序和启用状态；也是分类日报的收录边界 |
...
| 表 | 说明 |
|---|---|
| `daily_reports` | 日报日期、关键词分类 ID/名称快照、风险、结构化内容、模型、Prompt、Token 和状态 |
...
- 后端根据分数重新计算风险等级。
- 日报必须以单一关键词分类筛选输入；每个条目的引用 ID 必须在本次输入集合中，无有效引用的条目被剔除，并由后端为有效引用附加可信来源链接。
...
- `.env` 保存本地环境变量且不提交 Git。需要协助调整配置时，可临时创建不含真实值的 `.env.example` 用于核对，完成后删除。
...
| `ai_report_prompt` | 内置通用日报要求 | 全部单分类日报共用的分析与来源引用要求 |
| `ai_report_prompt_trade_policy` | 内置贸易政策要求 | 非关税政策、采购准入、原产地与本地化分析 |
| `ai_report_prompt_tariff_adjustment` | 内置关税要求 | 税率、品类、原产地、生效日、豁免与成本传导分析 |
| `ai_report_prompt_industry_regulation` | 内置行业法规要求 | 注册认证、标准、召回、进口警示与合规动作分析 |
...
| `ai_auto_report` | `false` | 自动处理后按活跃关键词分类分别生成当天日报 |
...
- 真相关与无关文章的分阶段调用次数。
- 正文失效后隐藏旧审核与旧业务结果。
- 日报按关键词分类只使用成功业务新闻，并执行来源校验及风险下限规则。
