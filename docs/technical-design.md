...
| 正文读取 | CCTQ Responses API + `gpt-5.4-mini` + `web_search` | 服务器端打开候选链接并返回正文 | 使用已验证能转发内置网页搜索的低成本模型；项目校验 `web_search_call`、成功标记、非空正文和最终 URL |
...
2. 每个关键词分类可同时配置核心组与拓展召回组；拓展组补充行业同义表达，并可要求标题或摘要真实命中主题词。
3. 前端调用 `/api/keywords/preview`，由后端实时返回查询表达式。
4. 保存时后端再次调用 `build_keyword_query()`，不接受客户端自由查询内容。
...
- 网页爬虫总开关默认关闭。关闭时普通 RSS 继续采集，显式爬虫源在任何页面请求前终止，RSS 返回 HTML 时也不会进入详情页爬取。
- 网页爬虫先缓存并检查同站 `robots.txt`，再按域名串行预留请求时间片。HTTP 429 使用 `Retry-After`，403/401、5xx、验证码和解析失败分别记录错误类型与冷却截止时间。
- 冷却只限制网页爬虫：RSS 仍可正常尝试；需要进入爬虫兜底时才跳过请求。受限组合记录失败且不推进游标，其他数据源继续运行。
...
- 候选链接、标题、发布方和发布时间一次性交给 CCTQ 的 GPT-5.4 mini 内置 `web_search`；请求强制 `tool_choice=required`，只有响应包含已完成的 `web_search_call`、模型返回成功且正文非空时才写库，短新闻不设最小字数门槛。
...
1. 用户选择北京时间日期。
2. 系统按新闻 `published_at` 筛选当日全部业务分析成功的新闻，不再按关键词分类过滤。
3. 系统将统一日报提示词、命中关键词、已保存的摘要、AI 业务分类、影响、风险和原文链接输入模型。
4. DeepSeek 跨主题比较当天事件，先返回 `overview` 总体概括，再返回按重要性排序的 `details` 事件解读；事实、业务意义、风险、机会和必要应对在同一事件正文中连贯表达。
5. 每条详细解读携带支持它的 `article_ids`；后端逐项丢弃不属于本次输入的 ID，没有任何有效来源的条目不写入日报。
6. 日报最终风险分数不得低于所选文章中的最高风险分数。
7. 后端根据有效文章 ID 为每条详细解读附加 `sources`，页面和飞书仅显示“来源 N”短超链接；日报内容与依据文章关系写入数据库。
8. 自动日报每天最多自动生成一份；当天没有合格文章时跳过。
...
| `keyword_categories` | 关键词分类名称、排序和启用状态；用于采集策略和相关性审核 |
| `keywords` | 所属关键词分类、关键词组、生成后的查询表达式、主题词、业务信号词、排除词、回溯天数、本地主题校验、启用和归档状态 |
...
| 表 | 说明 |
|---|---|
| `daily_reports` | 日报日期、风险、总体概括、详细解读、兼容旧字段、模型、Prompt、Token 和状态 |
...
每个批次内部采用阶段屏障：先并行完成正文抓取，再并行完成相关性审核，最后仅对审核通过的文章并行业务分析。三个阶段严格顺序执行，阶段内并发上限由 `ai_parallelism` 控制。暂停或上游限流时，已经开始的文章允许完成，尚未开始的文章继续保留为待办。
...
- 后端根据分数重新计算风险等级。
- 日报按自然日汇总全部合格输入，输出接口固定为总体概括和详细解读；每条解读的引用 ID 必须在本次输入集合中，无有效引用的条目被剔除，并由后端附加可信来源链接。
...
- 原有 SQLite 数据库文件仅用于迁移、测试或回退，不应提交到 Git。
...
- 模型明确返回失败、只有摘要或正文为空时不保存正文；单篇规范页暂时未找到时在当前批次即时重试一次。
...
- 真相关与无关文章的分阶段调用次数。
- 正文失效后隐藏旧审核与旧业务结果。
- 日报按自然日只使用成功业务新闻，并执行详细解读来源校验及风险下限规则。
