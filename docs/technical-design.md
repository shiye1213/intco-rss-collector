...
1. 用户选择北京时间日期。
2. 系统按新闻 `published_at` 筛选当日全部业务分析成功的新闻。
3. 系统使用通用日报提示词，将命中关键词、已保存的摘要、AI 业务分类、影响、风险和原文链接输入模型。
4. DeepSeek 返回管理层摘要，以及按业务方面设置小标题，并返回带业务分类和来源文章 ID 的事实变化、风险、机会、建议动作和观察清单。
5. 后端逐项丢弃不属于本次输入的 `article_id`；没有任何有效来源的结构化条目不写入日报。
6. 日报最终风险分数不得低于所选文章中的最高风险分数。
7. 后端根据有效文章 ID 为日报顶层和每个结构化条目附加发布方、标题和 `source_url`，日报内容与依据文章关系写入数据库。
8. 自动日报每天最多生成一份综合日报；所选日期没有合格文章时跳过。
...
| 表 | 说明 |
|---|---|
| `daily_reports` | 日报日期、风险、结构化内容、模型、Prompt、Token 和状态；分类字段仅保留用于兼容历史数据 |
...
- 后端根据分数重新计算风险等级。
- 日报汇总指定日期内全部成功业务新闻；每个条目的引用 ID 必须在本次输入集合中，无有效引用的条目被剔除，并由后端为有效引用附加可信来源链接。
...
| `ai_report_prompt` | 内置通用日报要求 | 综合日报的分析与来源引用要求 |
...
- 真相关与无关文章的分阶段调用次数。
- 正文失效后隐藏旧审核与旧业务结果。
- 日报按日期使用全部成功业务新闻，并执行来源校验及风险下限规则。
