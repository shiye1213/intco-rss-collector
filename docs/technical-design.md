...
| `app/query_builder.py` | 根据匹配词生成只含关键词的 Google News 查询表达式 |
...
1. 业务人员新增或修改关键词组，并按行填写匹配词。
2. 前端实时预览查询表达式。
3. 后端忽略客户端提交的自由查询内容，统一调用 `build_keyword_query()` 重新生成。
4. 表达式格式为 `("词1" OR "词2")`，不自动添加领域词或排除词。
...
| `keywords` | 关键词组、生成后的查询表达式、匹配词 JSON、启用和归档状态 |
